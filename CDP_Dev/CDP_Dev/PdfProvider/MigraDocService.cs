...
namespace AspNetCorePdf.PdfProvider
{
    public class MigraDocService : IMigraDocService
    {
        ...
        public string CreateMigraDocPdf(PdfData pdfData)
        {
            ...
            Document document = CreateCDPDocument(pdfData);
        ...
        private void DefineNewStyles(Document document)
        {
            try
            {
                // Get the predefined style Normal.
        ...
        private void DefineEnablementParagraphs(Document document, PdfData pdfData)
        {
            DemonstrateEnablementIndent(document, pdfData);
        }
        ...
        private void DefineMentorTables(Document document, PdfData pdfData)
        {
            DemonstrateMentorSimpleTable(document, pdfData);
        }
        private void DefineAllocationTables(Document document, PdfData pdfData)
        {
            DemonstrateAllocationSimpleTable(document, pdfData);
        }
        private void DefineInternalTables(Document document, PdfData pdfData)
        {
            DemonstrateInternalSimpleTable(document, pdfData);
        }
        ...
        private void DemonstrateAllocationSimpleTable(Document document, PdfData pdfData)
        {
            document.LastSection.AddParagraph("Allocation", "Heading2");

            Table table = new Table();
            table.Borders.Width = 0.75;

            Column column = table.AddColumn(Unit.FromCentimeter(9));
            column.Format.Alignment = ParagraphAlignment.Center;
            ...
            for (int i = 0; i < pdfData.Allocation.Count; i++)
            {
                row = table.AddRow();
                cell = row.Cells[0];
                cell.AddParagraph(pdfData.Allocation[i].Allocation);
                cell = row.Cells[1];
                cell.AddParagraph(pdfData.Allocation[i].Comment);
            }

            table.SetEdge(0, 0, 2, pdfData.Allocation.Count + 1, Edge.Box, BorderStyle.Single, 1.5, Colors.Black);

            document.LastSection.Add(table);
        }
        private void DemonstrateInternalSimpleTable(Document document, PdfData pdfData)
        {
            document.LastSection.AddParagraph("Internal", "Heading2");
            ...
            row.Shading.Color = Colors.Azure;
            Cell cell = row.Cells[0];
            cell.AddParagraph("Topic");
            cell = row.Cells[1];
            cell.AddParagraph("Training Mode");
            cell = row.Cells[2];
            ...
            for (int i = 0; i < pdfData.IInternal.Count; i++)
            {
                row = table.AddRow();
                cell = row.Cells[0];
                cell.AddParagraph(pdfData.IInternal[i].Topic);
                cell = row.Cells[1];
                cell.AddParagraph(pdfData.IInternal[i].TrainingMode);
                cell = row.Cells[2];
                cell.AddParagraph(pdfData.IInternal[i].Description);
            }

            table.SetEdge(0, 0, 3, pdfData.IInternal.Count + 1, Edge.Box, BorderStyle.Single, 1.5, Colors.Black);
